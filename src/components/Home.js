...
const API_BASE_URL =
    process.env.REACT_APP_API_BASE_URL || 'https://api.kasplex.org/v1'
...
const Home = () => {
    ...
    useEffect(() => {
        ...
        fetch(`${API_BASE_URL}/info`)
            ...
            .then(data => {
                ...
                setNetworkStats({
                    ...
                    tokenTotal: new Intl.NumberFormat('en-US').format(
                        tokenTotal,
                    ),
                    feeTotal: new Intl.NumberFormat('en-US').format(
                        (feeTotal / 1e8).toFixed(0),
                    ),
                })
            })
            .catch(error =>
        ...
        fetch(`${API_BASE_URL}/krc20/tokenlist`)
            .then(response => response.json())
            .then(data => {
                const sortedTokens = data.result.sort(
                    (a, b) => b.mtsAdd - a.mtsAdd,
                )
    ...
    return (
        <>
            ...
            <Container
                fluid
                className="home-container d-flex flex-column vh-100"
            >
                <SEO
                    title="Home"
                ...
                <Row className="mb-3">
                    <Col>
                        ...
                        <Row className="mb-2">
                            <FeatureCard
                                title="All Tokens"
                                icon={<FaCoins className="feature-icon" />}
                                link="/tokens"
                            />
                            <FeatureCard
                                title="Address Lookup"
                                icon={<FaWallet className="feature-icon" />}
                                link="/wallet"
                            />
                            <FeatureCard
