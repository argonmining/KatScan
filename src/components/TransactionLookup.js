...
const TransactionLookup = () => {
    ...
    const fetchTransactionData = async hash => {
        ...
        try {
            ...
            const commitResponse = await axios.get(
                `https://api.kaspa.org/transactions/${commitHash}`,
            )
            ...
            setError(
                'Failed to fetch transaction data. Please check the transaction hash and try again.',
            )
        } finally {
            setLoading(false)
        }
    ...
    const formatKRC20Amount = (amount, decimals) => {
        return (
            (parseInt(amount) / Math.pow(10, decimals)).toFixed(decimals) +
            ' ' +
            transactionData.krc20Data.tick
        )
    }

    const openExplorer = hash => {
        window.open(
            `https://explorer.kaspa.org/txs/${hash}`,
            '_blank',
            'noopener,noreferrer',
        )
    }
            ...
            <Accordion.Body>
                <Card className="mb-3">
                    <Card.Body>
                        <Row className="mb-2">
                            ...
                            <Col sm={8}>
                                <span
                                    className="clickable"
                                    onClick={() =>
                                        openExplorer(data.transaction_id)
                                    }
                                >
                        ...
                        <Row className="mb-2">
                            ...
                            <Col sm={8}>
                                <Badge
                                    bg={data.is_accepted ? 'success' : 'danger'}
                                    className="me-2"
                                >
                                    {data.is_accepted
                                        ? 'Accepted'
                        ...
                        <Row>
                            <Col sm={4}>
                                <strong>Block Blue Score</strong>
                            </Col>
                ...
                <Accordion>
                    {renderInputsOutputs(
                        data.inputs,
                        'Inputs',
                        `${isCommit ? 'commit' : 'reveal'}-inputs`,
                    )}
                    {renderInputsOutputs(
                        data.outputs,
                        'Outputs',
                        `${isCommit ? 'commit' : 'reveal'}-outputs`,
                    )}
                </Accordion>
            </Accordion.Body>
        </Accordion.Item>
    ...
    const renderInputsOutputs = (data, title, eventKey) => (
        <Accordion.Item eventKey={eventKey}>
            <Accordion.Header>
                <strong>{title}</strong>{' '}
                <span className="ms-2">({data.length})</span>
            </Accordion.Header>
            <Accordion.Body>
                {data.map((item, index) => (
                    <Card key={index} className="mb-3">
                        <Card.Body>
                            {item.previous_outpoint_hash && (
                                <Row className="mb-2">
                                    <Col sm={4}>
                                        <strong>Previous Outpoint Hash</strong>
                                    </Col>
                                    <Col sm={8}>
                                        <span
                            ...
                            {item.signature_script && (
                                <Row className="mb-2">
                                    <Col sm={4}>
                                        <strong>Signature Script</strong>
                                    </Col>
                                    <Col sm={8}>
        ...
        <Container className="transaction-lookup">
            ...
            <JsonLd
                data={{
                    ...
                    description:
                        'Explore detailed information about KRC-20 token transactions on the Kaspa blockchain.',
                    ...
                    <Card className="mb-4">
                        ...
                        <Card.Body>
                            ...
                            <Row className="mb-2">
                                ...
                                <Col sm={8}>
                                    {formatKaspa(
                                        transactionData.krc20Data.feeRev,
                                    )}
                                </Col>
                            </Row>
                            <Row>
                                ...
                                <Col sm={8}>
                                    {transactionData.krc20Data.opScore}
                                </Col>
                            </Row>
                        </Card.Body>
                    </Card>

                    <Card className="transaction-flow mb-4">
