...
const TokenComparison = () => {
    ...
    const calculateHolderPercentages = useCallback(
        token => {
            if (
                !token.holder ||
                !Array.isArray(token.holder) ||
            ...
            const sortedHolders = [...token.holder].sort(
            ...
            const top10Amount = sortedHolders
                ...
                .reduce(
                    (sum, holder) =>
                        sum + calculateValue(holder.amount, token.dec),
                    0,
                )
            const top50Amount = sortedHolders
                .slice(0, 50)
                .reduce(
                    (sum, holder) =>
                        sum + calculateValue(holder.amount, token.dec),
    ...
    const renderComparison = () => {
            ...
            datasets: [
                {
                    ...
                    data: [
                        token1.top10HoldersPercentage || 0,
                        token2.top10HoldersPercentage || 0,
                    ],
                ...
                },
                {
                    ...
                    data: [
                        (token1.top50HoldersPercentage || 0) -
                            (token1.top10HoldersPercentage || 0),
                        (token2.top50HoldersPercentage || 0) -
                            (token2.top10HoldersPercentage || 0),
                    ],
        ...
        const getMintType = token => {
            return calculateValue(token.pre, token.dec) > 0
                ? 'Pre-Mint'
                : 'Fair Mint'
        }
            ...
            <Row>
                ...
                <Col md={12} className="mb-4 minting-progress-card">
                    <Card>
                        <Card.Body>
                            <Card.Title>
                                <FaChartPie /> Minting Progress
                            </Card.Title>
                            <div className="progress-wrapper mb-3">
                                <div className="progress-label">
                                    {censorTicker(token1.tick)}{' '}
                                    <Badge
                                        bg={
                                            getMintType(token1) === 'Fair Mint'
                                                ? 'success'
                                                : 'warning'
                                        }
                                    >
                                ...
                                <ProgressBar
                                    ...
                                    label={`${getMintProgress(token1).toFixed(
                                        2,
                                    )}%`}
                                    variant="info"
                                />
                            ...
                            <div className="progress-wrapper">
                                <div className="progress-label">
                                    {censorTicker(token2.tick)}{' '}
                                    <Badge
                                        bg={
                                            getMintType(token2) === 'Fair Mint'
                                                ? 'success'
                                                : 'warning'
                                        }
                                    >
                                ...
                                <ProgressBar
                                    ...
                                    label={`${getMintProgress(token2).toFixed(
                                        2,
                                    )}%`}
                                    variant="success"
                                />
                            ...
                            <table className="table">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td>Total Holders</td>
                                        <td>
                                            {formatNumber(token1.holderTotal)}
                                        </td>
                                        <td>
                                            {formatNumber(token2.holderTotal)}
                                        </td>
                                    </tr>
                                    <tr>
        ...
        <div className="token-comparison">
            ...
            <JsonLd
                data={{
                    ...
                    description:
                        'Compare KRC-20 tokens on the Kaspa blockchain side by side, analyzing key metrics and holder distributions.',
            ...
            {error && (
                <div className="error-message">
                    <p>{error}</p>
                    <p>
                        Please try selecting the tokens again or contact support
